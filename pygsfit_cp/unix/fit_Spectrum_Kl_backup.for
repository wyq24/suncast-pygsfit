...
C         & FFitL(N_dat_in(3),N_dat_in(4),N_dat_in(5)),
        write(*,*) 'N_dat_in:', N_dat_in
        write(*,*) 'r_inp:', r_inp
        write(*,*) 'ParmsIn:', ParmsIn
        write(*,*) 'fr_in:', fr_in
             write(*,*) 'fr_in shape1:', size(fr_in, dim=1)
        ! 对于数组，你可能需要打印特定的元素或遍历数组
            !dims = size(spec_in)
            !shapeArray = shape(spec_in)

        write(*,*) 'spec_in shape1:', size(spec_in, dim=1)
        write(*,*) 'spec_in shape2:', size(spec_in, dim=2)
        write(*,*) 'spec_in shape3:', size(spec_in, dim=3)
        write(*,*) 'spec_in (first element):', spec_in
        write(*,*) 'ArrParms (first element):', ArrParms
        write(*,*) 'ErrParms (first element):', ErrParms
        write(*,*) 'spec_out (first element):', spec_out
C
...
!This reads arrays of the distribution function and 
              ...
              ErrFlcp(:,:)=spec_in(:,:,3)
              ErrFrcp(:,:)=spec_in(:,:,4)
              print *, 'spec_in(:,:,1)', spec_in(:,:,1)
              print *, 'spec_in(:,:,2)', spec_in(:,:,2)
              print *, 'Flcp', Flcp
              print *, 'Frcp', Frcp
